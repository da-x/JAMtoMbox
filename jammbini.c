...
int _JAMPROC JAMmbOpen(JAMAPIRECptr apirec)
{
	...
	if (apirec->
	    ReadFunc(apirec, apirec->HdrHandle, &apirec->HdrInfo,
		     (INT32) sizeof(JAMHDRINFO)) !=
	    (INT32) sizeof(JAMHDRINFO)) {
...
int _JAMPROC JAMmbClose(JAMAPIRECptr apirec)
{
	...
	if (apirec->isOpen) {
		...
		apirec->HdrHandle =
		    apirec->TxtHandle =
		    apirec->IdxHandle = apirec->LrdHandle = -1;
...
int _JAMPROC JAMmbCreate(JAMAPIRECptr apirec)
{
	...
	apirec->HdrInfo.DateCreated = JAMsysTime(NULL);
	apirec->HdrInfo.PasswordCRC = 0xffffffffL;
	apirec->HdrInfo.BaseMsgNum = 1L;

	if (apirec->
	    WriteFunc(apirec, apirec->HdrHandle, &apirec->HdrInfo,
		      (INT32) sizeof(JAMHDRINFO)) !=
	    (INT32) sizeof(JAMHDRINFO)) {
		apirec->CloseFunc(apirec, apirec->HdrHandle);
		return (0);
...
{
	CHAR8 FileName[200], JunkBuf[110];
	JAMIDXREC IdxRec;
	JAMSUBFIELD *SubPtr;
	...
	while (!Done && !Error) {
		...
		if (apirec->
			...
			/* Read header */
			Junk =
			    apirec->ReadFunc(apirec, apirec->HdrHandle,
					     &apirec->Hdr,
					     (INT32) sizeof(JAMHDR));
			if (Junk != (INT32) sizeof(JAMHDR)) {
				...
				if (apirec->Hdr.SubfieldLen
				    && !(apirec->Hdr.Attribute & MSG_DELETED)) {
					FoundField = 0;
					JunkLong = 0L;
					do {
						Junk =
						    apirec->ReadFunc(apirec,
								     apirec->
								     HdrHandle,
								     &JunkBuf,
								     sizeof
					...
					while (!Error && !FoundField && !Done
					       && (INT32) JunkLong <
					       (INT32) apirec->Hdr.SubfieldLen);

					if (FoundField) {
						*((UCHAR8 _JAMFAR
						   *) (JAMsysAddFarPtr(JunkBuf,
								       SubDatLen)))
				     = '\0';
						strlwr(JunkBuf);
						IdxRec.UserCRC =
						    JAMsysCrc32((void _JAMFAR *)
								&JunkBuf,
								(UINT16)
								SubDatLen,
								0xffffffffL);
				...
				if (apirec->Hdr.MsgNum >=
				    apirec->HdrInfo.BaseMsgNum) {
					if (!Error) {
						NextIdxOffset =
						    (apirec->Hdr.MsgNum -
						     apirec->HdrInfo.
