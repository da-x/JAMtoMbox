...
*/
	typedef struct _JAMAPIREC {
		...
		/* Functions */
		 ...
		 FHANDLE(_JAMPROC * CreateFunc) (struct _JAMAPIREC _JAMDATA *,
						 CHAR8ptr FileName);
		 FHANDLE(_JAMPROC * OpenFunc) (struct _JAMAPIREC _JAMDATA *,
					       CHAR8ptr FileName);
		int (_JAMPROC * CloseFunc) (struct _JAMAPIREC _JAMDATA *,
					    FHANDLE Handle);
		 INT32(_JAMPROC * ReadFunc) (struct _JAMAPIREC _JAMDATA *,
					     FHANDLE Handle, VOIDptr Buffer,
					     INT32 Length);
		 INT32(_JAMPROC * WriteFunc) (struct _JAMAPIREC _JAMDATA *,
					      FHANDLE Handle, VOIDptr Buffer,
					      INT32 Length);
		 INT32(_JAMPROC * SeekFunc) (struct _JAMAPIREC _JAMDATA *,
					     FHANDLE Handle, int FromWhere,
					     INT32 Offset);
		int (_JAMPROC * LockFunc) (struct _JAMAPIREC _JAMDATA *,
					   int DoLock);
		int (_JAMPROC * UnlinkFunc) (struct _JAMAPIREC _JAMDATA *,
					     CHAR8ptr FileName);
...
*/
	...
	int _JAMPROC JAMmbStoreMsgTxtBuf(JAMAPIRECptr apirec, CHAR8ptr, INT32,
					 int);
