...
int _JAMPROC JAMmbFetchLastRead(JAMAPIRECptr apirec, UINT32 UserID)
{
	...
	while (1) {
		ReadCount =
		    apirec->ReadFunc(apirec, apirec->LrdHandle,
				     &apirec->LastRead,
				     (INT32) sizeof(JAMLREAD));
...
int _JAMPROC JAMmbStoreLastRead(JAMAPIRECptr apirec, int UpdateHdrInfo)
{
	...
	if (apirec->
	    SeekFunc(apirec, apirec->LrdHandle, JAMSEEK_SET,
	...
	/* Write record */
	if (apirec->
	    WriteFunc(apirec, apirec->LrdHandle, &apirec->LastRead,
		      (INT32) sizeof(JAMLREAD)) != (INT32) sizeof(JAMLREAD)) {
