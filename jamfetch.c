...
**  failure.
*/
int _JAMPROC JAMmbFetchMsgHdr(JAMAPIRECptr apirec, UINT32 WhatMsg,
			      int WithSubFields)
{
	INT32 ReadCount;
	...
	/* Fetch header */
	if (apirec->
	    SeekFunc(apirec, apirec->HdrHandle, JAMSEEK_SET,
		     (INT32) apirec->Idx.HdrOffset) !=
	    (INT32) apirec->Idx.HdrOffset) {
		apirec->APImsg = JAMAPIMSG_SEEKERROR;
		return (0);
	}
	if (apirec->
	    ReadFunc(apirec, apirec->HdrHandle, &apirec->Hdr,
		     (INT32) sizeof(JAMHDR)) != (INT32) sizeof(JAMHDR)) {
	...
	if (ReadCount) {
		if (apirec->
		    ReadFunc(apirec, apirec->HdrHandle, apirec->WorkBuf,
			     ReadCount) != ReadCount) {
			apirec->APImsg = JAMAPIMSG_CANTRDFILE;
			return (0);
		}
...
int _JAMPROC JAMmbFetchMsgIdx(JAMAPIRECptr apirec, UINT32 WhatMsg)
{
	...
	WhatOffset =
	    (INT32) ((WhatMsg -
		      apirec->HdrInfo.BaseMsgNum) * (INT32) sizeof(JAMIDXREC));
	if (apirec->
	    SeekFunc(apirec, apirec->IdxHandle, JAMSEEK_SET,
		     WhatOffset) != WhatOffset) {
		apirec->APImsg = JAMAPIMSG_SEEKERROR;
		return (0);
	}
	if (apirec->
	    ReadFunc(apirec, apirec->IdxHandle, &apirec->Idx,
		     (INT32) sizeof(JAMIDXREC)) != (INT32) sizeof(JAMIDXREC)) {
		apirec->APImsg = JAMAPIMSG_CANTRDFILE;
		return (0);
	}
...
{
	return (JAMmbFetchMsgHdr
		(apirec, (apirec->LastMsgNum + 1L), WithSubFields));
}
...
int _JAMPROC JAMmbFetchMsgTxt(JAMAPIRECptr apirec, int FirstFetch)
{
	...
	if (FirstFetch) {
		if (apirec->
		    SeekFunc(apirec, apirec->TxtHandle, JAMSEEK_SET,
			     (INT32) apirec->Hdr.TxtOffset) !=
		    (INT32) apirec->Hdr.TxtOffset) {
			apirec->APImsg = JAMAPIMSG_SEEKERROR;
			return (0);
		} else
	...
	/* Get it from disk */
	if (apirec->
	    ReadFunc(apirec, apirec->TxtHandle, apirec->WorkBuf,
		     ReadCount) != ReadCount) {
		apirec->APImsg = JAMAPIMSG_CANTRDFILE;
		return (0);
	}
