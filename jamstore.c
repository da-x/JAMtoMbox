...
int _JAMPROC JAMmbStoreMsgHdr(JAMAPIRECptr apirec, UINT32 WhatMsg)
{
	...
	if (apirec->
	    WriteFunc(apirec, apirec->HdrHandle, &apirec->Hdr,
...
int _JAMPROC JAMmbStoreMsgIdx(JAMAPIRECptr apirec, UINT32 WhatMsg)
{
	...
	if (apirec->
	    WriteFunc(apirec, apirec->IdxHandle, &apirec->Idx,
...
int _JAMPROC JAMmbStoreMsgTxt(JAMAPIRECptr apirec)
{
	...
	if (apirec->
	    WriteFunc(apirec, apirec->TxtHandle, apirec->WorkBuf,
		      (INT32) apirec->Hdr.TxtLen) !=
	    (INT32) apirec->Hdr.TxtLen) {
		apirec->APImsg = JAMAPIMSG_CANTWRFILE;
		return (0);
	}
				 ...
				 INT32 BufLen, int IsFirst)
{
	...
	/* Write the text */
	if (apirec->WriteFunc(apirec, apirec->TxtHandle, Buffer, BufLen) !=
	    BufLen) {
		apirec->APImsg = JAMAPIMSG_CANTWRFILE;
		return (0);
	}
